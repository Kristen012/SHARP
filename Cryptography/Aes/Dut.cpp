...
void Dut::assign_key(U8 key_in[SIZE], U8 key_out[SIZE])
{
    for (unsigned int i = 0; i < SIZE; i++)
        key_out[i] = key_in[i];
}

void Dut::SubBytes(U8 state[SIZE])
{
    for (unsigned int i = 0; i < SIZE; i++)
        state[i] = sbox[state[i]];
}

void Dut::ShiftRows(U8 state[SIZE])
{
    for (unsigned int row = 1; row < 4; row++)
    {
        for (unsigned int counter = 0; counter < row; counter++)
        {
            unsigned int row_ind = Nb * row;
...
void Dut::MixColumns(U8 state[SIZE])
{
    U8 state_tmp[4][Nb];
    U8 col_tmp[4];
    U8 x_2, x_3, x_1;
    ...
    for (unsigned int row = 0; row < 4; row++)
        for (unsigned int col = 0; col < Nb; col++)
...
U8 Dut::xadd(U8 in1, U8 in2)
{
    return (in1 ^ in2);
}

U8 Dut::xtime_3(U8 data)
{
    U8 x_2 = xtime_2(data);
    return xadd(x_2, data);
}

unsigned int Dut::clip(unsigned int n)
{
    if (n > 3)
        n = n - 4;
    return n;
...
void Dut::KeyExpansion(U8 current_key[SIZE], U8 rcon, U8 next_key[SIZE])
{
    U8 key_in_tmp[4][Nb];
    U8 key_out_tmp[4][Nb];
...
void Dut::AddRoundKey(U8 state[SIZE], U8 key[SIZE])
{
    for (unsigned int i = 0; i < SIZE; i++)
...
void Dut::do_compute() {
    ...
    while (true) {
		...
		for (unsigned int i = 0; i < SIZE; i++) {
#ifndef NATIVE_SYSTEMC
			o_result.put(idata_buf[i]);
#else
			o_result.write(idata_buf[i]);
#endif
		}
    }
}
