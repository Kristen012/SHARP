...
void Testbench::do_feed() {
	...
	FILE* x3_fp = fopen("../data/input_x3.dat", "r");
	if (!x3_fp) {
		std::cerr << "[TB] ERROR: Could not open input_x3.dat!" << std::endl;
		exit(1);
	}
	for (int i = 0; i < X_SIZE; i++) {
		fscanf(x3_fp, "%d", &input_x3[i]);
	}
	fclose(x3_fp);

	FILE* x4_fp = fopen("../data/input_x4.dat", "r");
	if (!x4_fp) {
		std::cerr << "[TB] ERROR: Could not open input_x4.dat!" << std::endl;
		exit(1);
	}
	for (int i = 0; i < X_SIZE; i++) {
		fscanf(x4_fp, "%d", &input_x4[i]);
	}
	fclose(x4_fp);
	...
	for (int i = 0; i < X_SIZE; i++) {
		...
		i_x3.write(input_x3[i]);
		i_x4.write(input_x4[i]);
#endif
	}
}
...
#ifndef NATIVE_SYSTEMC
		output_t inVal = o_outp.get();
#else
		output_t inVal = o_outp.read();
#endif
		unsigned v = inVal.to_uint();
		for (int bit = 3; bit >= 0; --bit) {
			fprintf(outfp, "%d", (v >> bit) & 1);
		}
		fprintf(outfp, "\n");
    }
